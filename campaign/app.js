...
// Serves this folder as static files and stores modal submissions in SQLite.
// No npm dependencies — node:sqlite is built into Node 22.5+ / 24.
//
// cPanel → Setup Node.js App:
//   Application root        : the folder containing this file
//   Application startup file: app.js
//   Environment variables   : ADMIN_PASSWORD = <your password>
//
// The database lives OUTSIDE this folder (see DATA_DIR below) so it can never
// be downloaded over HTTP, even if the Node app is stopped and Apache falls
// back to serving these files statically.
...
// Sibling of the web root, so it is not reachable over HTTP.
const DATA_DIR = process.env.DATA_DIR || path.resolve(__dirname, '..', 'campaign-data');
...
try {
    ...
    console.error(`Cannot create the data directory: ${DATA_DIR}`);
    console.error(`(${err.message}) Set DATA_DIR to a writable folder outside the web root.`);
    process.exit(1);
}
...
const insertStmt = db.prepare(`
    INSERT INTO submissions (form, name, phone, email, message, quantity, terms, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
`);
const selectAllStmt = db.prepare('SELECT * FROM submissions ORDER BY id DESC');
...
function renderAdmin(res) {
    ...
    const body = rows
        .map(
            ...
            <tr>
                <td>${row.id}</td>
                <td class="nowrap">${escapeHtml(new Date(row.created_at).toLocaleString('vi-VN'))}</td>
...
const server = http.createServer((req, res) => {
    ...
    if (urlPath === '/admin' || urlPath === '/admin/') {
        if (requireAuth(req, res)) renderAdmin(res);
        return;
    }

    if (urlPath === '/admin/export.csv') {
        if (requireAuth(req, res)) exportCsv(res);
        return;
    }
